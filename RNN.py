...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.model = nn.RNN(hidden_size, hidden_size)
...
def trainer(x_train, y_train, encoder, decoder, epochs, lr=0.01):
    if not os.path.exists("./RNN-model"):
        os.makedirs("./RNN-model")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if prev_loss is None or total_loss < prev_loss:
            ...
            torch.save(encoder.state_dict(), "./RNN-model/encoder.pt")
            torch.save(decoder.state_dict(), "./RNN-model/decoder.pt")
...
encoder.load_state_dict(torch.load("./RNN-model/encoder.pt"))
attn_decoder.load_state_dict(torch.load("./RNN-model/decoder.pt"))
