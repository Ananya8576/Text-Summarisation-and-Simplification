...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.model = nn.LSTM(hidden_size, hidden_size)
        # self.model = nn.GRU(hidden_size, hidden_size, bidirectional=False)
        # self.model = nn.LSTM(hidden_size, hidden_size, bidirectional=False)
...
class AttnDecoder(nn.Module):
    def __init__(self, hidden_size, output_size, dropout_prob=0.2, max_length=300):
    ...
    def forward(self, input, hidden, encoder_output):
        ...
        attn_weights = functional.softmax(self.attn(torch.cat((embedded[0], hidden[0][0]), 1)), dim=1)
...
def trainer(x_train, y_train, encoder, decoder, epochs, lr=0.01):
    if not os.path.exists("./LSTM-model"):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if prev_loss is None or total_loss < prev_loss:
            ...
            torch.save(encoder.state_dict(), "./LSTM-model/encoder.pt")
            torch.save(decoder.state_dict(), "./LSTM-model/decoder.pt")
...
if not os.path.exists("./BiLSTM-model"):
    os.makedirs("./BiLSTM-model")

if os.path.exists("./BiLSTM-model/tokenizer.json"):
    with open('./BiLSTM-model/tokenizer.json') as f:
        data = json.load(f)
    ...
    with io.open('./BiLSTM-model/tokenizer.json', 'w', encoding='utf-8') as f:
...
encoder.load_state_dict(torch.load("./LSTM-model/encoder.pt"))
attn_decoder.load_state_dict(torch.load("./LSTM-model/decoder.pt"))
...
bleu =sentence_bleu([' '.join(x for x in generated).split()], ' '.join(x for x in Y_extra).split())
